...
| 6 | Voice | `in-progress` | **Code complete + reviewed + env smoke-verified; awaiting user mic gate.** Built on branch `phase6-voice` (subagent-driven TDD, 8 tasks + fix wave, all reviews clean, 23/23 web tests, vite build OK). Always-on wake-word assistant: browser streams 16 kHz PCM over `WS /api/voice`; sidecar runs **openWakeWord** (`hey_jarvis`) while sleeping, then **faster-whisper** (`small`/CPU) STT → reuses Phase-1 `stream_chat` router → **Mark-XL Edge-TTS** mp3, sentence-streamed; idle-timeout sleep + barge-in (`{type:"cancel"}` + browser mic-RMS stop). Zero backbone/vendor edits — new `scripts/jarvis_voice.py` (STT/TTS reuse) + `scripts/jarvis_wake.py` (`WakeWord`+`Segmenter` webrtcvad/energy) + `voice_ws` + `web/` Voice tab (`VoiceTab.jsx`, `voiceSocket.js`, `pcm-worklet.js`). Smoke confirmed real path: TTS 14 KB mp3, wake loads `hey_jarvis`, VAD=webrtcvad, STT healthy. **FOLLOW-UP (user):** run the mic gate (below); then merge `phase6-voice`→`main`. |
