...
| 8 | C — ada-style shell & UI | `passed` | **GATE met — user-confirmed live ("everything works").** Code complete + automated gate green. Left-rail layout replaced by ada's shell: a fixed **bottom feature bar** opens floating, **draggable module windows** over a central `ReactorCore` HUD; all 8 existing tabs wrapped unmodified as windows. New `web/src/shell/` (`modules.js` registry, `useWindows.js` open/close/focus/z-order + debounced server persist, `ModuleWindow.jsx` ada-ported pointer-drag + viewport clamp, `BottomBar.jsx`) + `lib/layoutApi.js`; `App.jsx` rewritten. Layout persists via new sidecar `GET/PUT /api/ui-layout` (mirrors `/api/board`: atomic temp+replace, corrupt→empty, validate-before-write, **no localStorage**) — zero backbone/`_vendor` edits. Tests: **8/8** `test_ui_layout.py` + **6/6** Playwright `adaShell.spec.js` (bottom bar, open, multi-window coexist, drag moves, saved layout restores, Chat input renders inside window); vite build OK (1583 modules). Built brainstorm→spec→plan; Task-1 backend reviewed (Approved, `z`-required hardening applied); Tasks 2-7 inline transcription. Spec `docs/superpowers/specs/2026-06-21-phase8-ada-shell-design.md`, plan `docs/superpowers/plans/2026-06-21-phase8-ada-shell.md`. **GATE (user):** run launcher → bottom bar → click icons open draggable windows → multiple coexist → drag one → reload restores layout → each tab works inside its window. |
| 9 | B+ — Agent graph (ECC personas + agent-to-agent talk) | `passed` | **GATE met — user-confirmed live ("gate passed").** Code complete + tested (full web suite green: 95 pytest + Playwright `agentGraph.spec.js` 2/2; vite build 1583 modules). Upgrades the Agent Board so the user **draws agent→agent edges** (not just agent→orchestrator) and the graph runs in **topological order** with a **≤3-concurrent** guard. New `scripts/jarvis_graph.py` = the only novel logic: `_validate_graph` (rejects cycles via Kahn, requires the `orchestrator` mandatory terminal sink — every agent must reach it — `.env`-only models, `MAX_NODES=16`) + async `run_graph` (one `asyncio.Task` per node awaits its predecessors then holds a `Semaphore(3)`, so dependents wait and ≤3 LLM calls are ever in flight) + node-id-keyed frame protocol (`node_start/node_chunk/node_end/edge_flow/graph_done/error`). **Wire-don't-rewrite:** reuses `jarvis_council._astream/_ladder_from/_nim/_known_models/synthesis_directive/THINKING_ON` + `jarvis_router.complete_with_fallback` (orchestrator = reasoning model + `enable_thinking`; per-node call is an injectable `call_node` test seam). New `scripts/gen_personas.py` build-time generator emits committed `web/src/lib/agents.js` (34 personas across Core/Reviewers/Builders/Architects/Domain/Research) + `scripts/personas.json` (id set the sidecar's `BOARD_PERSONAS` now loads, fallback to the 8). Sidecar `WS /api/graph` mirrors `council_ws` (thread+queue bridge). Frontend: `nodesConnectable` + `onConnect` user edges, `graphSocket.js`, id-keyed frame mapping replacing the `PROPOSAL <n>` index match, grouped palette, edge-flow pulses, drawn-edge persistence — zero backbone/`_vendor` edits. Spec/plan in `docs/superpowers/` (gitignored). Branch `phase9-agent-graph`. **GATE (user):** draw proposers→Fact-checker→Orchestrator → run → nodes stream in topological order, drawn lines pulse as outputs flow, Fact-checker consumes proposers before the Orchestrator synthesizes, ≤3 concurrent, layout persists across reload. |
| 10 | A — Vision & camera | `gate pending (user)` | Code complete + tested on branch `phase10-vision-camera`. Browser-side MediaPipe (`@mediapipe/tasks-vision`) extracts face/hand landmarks; sidecar owns only the ported cosine-compare + enrolled vector (presence-only, **never returned**). New `scripts/jarvis_vision.py` (`compare_landmarks` ported from ada `authenticator.py:86-108` + `FaceReferenceStore` + `LockStore` + `validate_vector`, pure numpy) + 4 `/api/vision/*` endpoints (`status`/`enroll`/`verify`/`lock`). Frontend: `lib/{faceMesh,handGestures,visionApi}.js`, `AuthLock.jsx` (full-screen themed unlock overlay, re-themed to HUD), `VisionTab.jsx` module (enroll / face-lock toggle / gesture control / mic visualizer), `Visualizer.jsx` (ported), `App.jsx` startup gate + gesture window-control (Open Palm→Chat, Fist→close focused, Peace→Voice, Pinch→Vision), `vision` registered in `shell/modules.js`+`UI_MODULES`. Models vendored to `web/public/mediapipe/` (gitignored) via `scripts/fetch_vision_models.ps1`. Tests: pytest `test_vision.py` **8/8** (full web suite 103/103) + Playwright `vision.spec.js` **4/4** (injected fake recognizer — no camera/WASM in CI); vite build OK (1946 modules). Zero backbone/`_vendor` edits. **GATE (user, needs camera+mic):** run launcher → Vision module → Enroll face → Face-lock ON → reload unlocks on your face; gesture control fires the mapped window actions; visualizer reacts to mic. |
...
| 11 (D) | D — Web-safe action tools | `passed (auto); gate verified live` | **GATE met live** — `jarvis ask --agent operative "weather in London"` → orchestrator invoked the ported **weather** tool (open-meteo): "clear sky, 26.2°C, wind 17.6 km/h"; and a web-search query → **web_search** tool (DDG + router-summarized via NIM) returned a real answer. Ported six Mark-XL actions into ONE new stdio MCP server `scripts/jarvis_tools_mcp.py` (FastMCP): `web_search`/`weather`/`reminder`/`youtube`/`flight_finder`/`file_processor`. **Wire-don't-rewrite:** reused each action's kernel and swapped its internal `core.llm_client.call_llm_text` for a `_route_llm` shim → Phase-1 `complete_with_fallback` (the "no new LLM path" linchpin). Web-safe adaptations: weather **rewritten** to open-meteo text (no browser); youtube drops tkinter+browser-open (`play` returns the URL, `summarize` takes `url`); flight scrapes via **Playwright** (not Selenium); reminder runs on the sidecar host; file_processor handles text/docx/json/code (heavy deps optional). Registered via `setup_config._tools_server()` (3rd MCP server) — `verify_mcp.py` now discovers **21 tools** through the real OpenJarvis loader (6 jarvis-tools). Tools auto-appear in the existing ToolsTab discover view → **no frontend**. Tests: `test_tools_mcp.py` **50/50** (full web suite **153/153**), coverage **82%**, all network+LLM mocked (zero live spend). Zero backbone/`_vendor` edits. Branch `phase11-action-tools`. **NOTE:** reminder not fired live (would register a real OS scheduled task); F (Clicky) is a separate later slice. |
| 11 (F) | F — Clicky (screen-aware pointing) | `code complete; live gate needs VISION_MODEL` | Screen-aware Q&A + on-screen pointing. New `scripts/jarvis_clicky.py` ports clicky-windows' **two-stage grid (Set-of-Mark) pointing** from `_vendor/clicky-windows/ai/universal_locator.py` (12×8 → crop 3×3 → 6×6 → map cell-center to px) + mss capture from Mark-XL `screen_processor.py`. **Wire-don't-rewrite:** the async streaming-provider seam is replaced by a sync `_vision_ask` that rides `LiteLLMEngine.generate` with **OpenAI image blocks** (`messages_to_dicts` forwards `content` verbatim → a NIM VLM gets `image_url`) — no new LLM path. Model id from `.env` **`VISION_MODEL`** (prefixed `nvidia_nim/` if bare; never hardcoded); unset → graceful "set VISION_MODEL" message. Sidecar `POST /api/clicky/point` (runs blocking capture+vision in `asyncio.to_thread`); `clicky` added to `UI_MODULES`. Frontend `ClickyTab.jsx` + `lib/clickyApi.js`, registered in `shell/modules.js` (Crosshair icon). Tests: `test_clicky.py` **19/19** (84% core coverage, vision+capture injected — offline) + 2 sidecar endpoint tests; full web suite **174/174**; vite build OK (1948 modules). Cloned `_vendor/clicky-windows` (read-only). Zero backbone/`_vendor` edits. Branch `phase11-action-tools`. **GATE (user):** set a NIM `VISION_MODEL` in `.env` → open Clicky → "where is the Save button" → annotated screenshot with the region highlighted + answer. |
...
- **2026-06-21 (session 8):** **Phase 9 (B+ — Agent graph)** built via brainstorm→spec→plan→inline-TDD
  on branch `phase9-agent-graph` (8 tasks, one commit each, all reviews green). Read
  `jarvis_council.py`/`jarvis_web_api.py`/`AgentsTab.jsx`+`board/*` first → the graph is the council
  *generalized to a DAG*, so it reuses the council's streaming/ladder/synthesis primitives and the only
  ...
  `Semaphore(3)` — dependents wait, ≤3 calls ever in flight; the free-tier 429 guard) + node-id-keyed
  ...
  `scripts/gen_personas.py` build-time generator → committed `web/src/lib/agents.js` (34 personas, 6
  ...
  managed edges with `nodesConnectable`+`onConnect` (drop auto-wires agent→orchestrator, user rewires to
  agent→agent), new `graphSocket.js`, id-keyed frame mapping (replaces the `PROPOSAL <n>` index match),
  category-grouped palette, edge-flow pulses, drawn-edge persistence — zero backbone/`_vendor` edits.
  ...
  mock the frame protocol → deterministic, launcher-free) + vite build OK (1583 modules). Two test-design
  ...
  **GATE met** — user confirmed the live run ("gate passed"): drew proposers→Fact-checker→Orchestrator,
  saw topological streaming + edge pulses + the Fact-checker consuming proposers before synthesis, layout
  persisted. Branch `phase9-agent-graph` ready to merge to `main` (pending user go). v2 ideas noted in the
  spec: edge *types* (review/critique/debate) and bounded debate loops (cycles with a max-iteration cap).

- **2026-06-22 (session 9):** **Phase 10 (A — Vision & camera)** built via brainstorm→spec→plan→subagent/inline
  on branch `phase10-vision-camera`. Decisions (user): browser extracts landmarks (MediaPipe WASM) / server
  decides; all three sub-features (face-auth + gesture + visualizer); themed UX unlock; full-screen startup
  overlay; window-control gestures. ada's `authenticator.py` is macOS-only (`CAP_AVFOUNDATION`) and opens the
  camera server-side → only its ~25-line cosine-compare is reused (`scripts/jarvis_vision.py`, pure numpy,
  pytest 8/8); the enrolled vector is treated like a secret (presence-only, never returned). Browser libs
  `lib/{faceMesh,handGestures,visionApi}.js` behind injectable factory seams (+ `window.__VISION_FAKE__`) so
  Playwright tests run with no camera/WASM (4/4). `AuthLock.jsx` (full-screen themed unlock) + `VisionTab.jsx`
  (enroll/lock/gesture/visualizer) + `Visualizer.jsx` (ported, A.D.A→JARVIS) + `App.jsx` startup gate & gesture
  dispatch. Models vendored to `web/public/mediapipe/` (gitignored) via `scripts/fetch_vision_models.ps1`.
  Tasks 1-2 subagent-driven (spec✅/quality Approved); Tasks 3-7 inline (subagents lack npm/pwsh permission;
  frontend needs npx playwright + Vite dev). Full web suite 103/103; vite build 1946 modules. One layout fix:
  visualizer wrapper `pointer-events-none`+`overflow-hidden` (decorative canvas was intercepting a button click).
  Zero backbone/`_vendor` edits. **GATE pending user** (needs camera+mic). Note: `chat.spec.js` live-round-trip
  test needs the running sidecar+NIM (not a Phase-10 change).
...
- `uv run python scripts/verify_mcp.py` — Phase 2/11 gate: discover MCP tools via the real loader (now 21 tools incl. 6 jarvis-tools). Set `PYTHONUTF8=1`.
- `uv run pytest tests/web/test_tools_mcp.py -q` — Phase 11 (slice D) action-tools unit tests (50; network+LLM mocked).
- `& .\scripts\jarvis.ps1 ask --agent operative "what's the weather in London"` — Phase 11 live gate: orchestrator invokes a ported tool.
...
- `pwsh .\scripts\jarvis_web.ps1` — Phase 5 Web UI + Phase 6 Voice + Phase 9 Agent graph: starts sidecar (:8700) + Vite dev (:5173), loads `.env`.
- `uv run python scripts/jarvis_graph.py "<task>"` — Phase 9 agent graph (headless): builds the flat council-parity graph + runs it in topological order.
- `uv run python scripts/jarvis_graph.py --graph path/to/graph.json` — run an arbitrary `{task,nodes,edges}` DAG headless.
- `uv run python scripts/gen_personas.py` — regenerate the committed persona catalog (`web/src/lib/agents.js` + `scripts/personas.json`).
- `uv run pytest tests/web/test_graph.py tests/web/test_graph_ws.py tests/web/test_personas.py -q` — Phase 9 unit + WS + catalog tests.
- `cd web; npx playwright test agentGraph.spec.js` — Phase 9 board graph spec (seeded chain render + mocked frame-protocol run; needs Vite dev on :5173).
- `pwsh scripts/fetch_vision_models.ps1` — Phase 10 one-time: download MediaPipe wasm + face/hand models into `web/public/mediapipe/` (gitignored).
- `uv run pytest tests/web/test_vision.py -q` — Phase 10 vision core + endpoint tests (8).
- `cd web; npx playwright test vision.spec.js` — Phase 10 vision specs (gesture classifier + AuthLock unlock + gesture→window + visualizer; injected fake recognizer, needs Vite dev on :5173).
